...
/-- If `x` is a non-identity element of a 2-group with `orderOf x = 2 ^ k` and `k ≤ 4`,
and the group has no element of order 8, then `orderOf x = 2` or `orderOf x = 4`. -/
private lemma orderOf_2_or_4_of_no8 {G : Type*} [Group G] [Finite G] {x : G} {k : ℕ}
    (hk : orderOf x = 2 ^ k)
    (hk_le : k ≤ 4)
    (h_ne_one : x ≠ 1)
    (h_no8 : ¬ ∃ y : G, orderOf y = 8) :
    orderOf x = 2 ∨ orderOf x = 4 := by
  interval_cases k
  · exfalso; exact h_ne_one (orderOf_eq_one_iff.mp hk)
  · left; exact hk
  · right; exact hk
  · exfalso; exact h_no8 ⟨x, hk⟩
  · exfalso; apply h_no8
    refine ⟨x ^ 2, ?_⟩; rw [orderOf_pow, hk]; decide

/-- If every element of a group of order 16 squares to 1, the group is isomorphic to C₂⁴,
contradicting `h_non_iso`. -/
private lemma exponent_two_iso_C24 {G : Type*} [Group G] [Finite G]
    (hn : Nat.card G = 16)
    (h_non_iso : IsEmpty (G ≃* CyclicGroup 2 × CyclicGroup 2 × CyclicGroup 2 × CyclicGroup 2))
    (h_sq : ∀ x : G, x ^ 2 = 1) : False := by
  ...
  let pi_iso : (Fin 4 → CyclicGroup 2) ≃*
    ...
    { toFun := fun f => (f 0, f 1, f 2, f 3)
      invFun := fun p =>
        Fin.cons p.1 (Fin.cons p.2.1 (Fin.cons p.2.2.1 (Fin.cons p.2.2.2 Fin.elim0)))
      left_inv := fun f => by funext i; fin_cases i <;> rfl
      right_inv := fun p => rfl
      map_mul' := fun f g => by ext <;> rfl }
  exact h_non_iso.false (e.trans pi_iso)

/-- Given a central element `z` of order 2 and an element `x` of order 4 with `x^2 ≠ z`,
construct a normal subgroup `K ≅ C₄ × C₂` of index 2. -/
private lemma exists_K8_normal_subgroup {G : Type*} [Group G]
    (hn : Nat.card G = 16)
    (z : G) (hz : orderOf z = 2) (hz_center : z ∈ Subgroup.center G)
    (x : G) (hxord : orderOf x = 4) (hxsq : x ^ 2 ≠ z) :
    ∃ K : Subgroup G, K.Normal ∧ Nonempty (K ≃* CyclicGroup 4 × CyclicGroup 2) := by
  ...
  · have h_K_card : Nat.card K = 8 := by
      have h_LH_card : Nat.card ↥(L ⊔ H) = 8 := by
        ...
        rw [hLc, hHc] at hLH; omega
      rw [show K = L ⊔ H from hK_eq]; exact h_LH_card
    have h_idx : K.index = 2 := by
      have := Subgroup.index_mul_card K
      rw [h_K_card, hn] at this; omega
  ...
  · constructor
    ...
    letI iH : H ≃* CyclicGroup 2 := mulEquivOfCyclicCardEq (
        by simp only [H, Nat.card_zpowers, hz, card_cyclicGroup])
    letI iL : L ≃* CyclicGroup 4 := mulEquivOfCyclicCardEq (
        by simp only [L, Nat.card_zpowers, hxord, card_cyclicGroup])
    let prod_iso : L × H ≃* ↑(L ⊔ H) := mulEquiv_sup_of_disjoint_comm L H h_disj' h_comm
    let eq_iso : K ≃* ↑(L ⊔ H) := MulEquiv.subgroupCongr hK_eq
...
lemma exists_normal_C8_or_C4_C2
  ...
  · -- There are no elements of order 8, then K8 ◃ G
    ...
    have h_max_order : ∃ z : G , orderOf z = 4 := by
      ...
      exact exponent_two_iso_C24 hn h_non_iso h_sq
    ...
    · -- There is x of order 4 such that x^2 ≠ z; use the K₈ helper
      obtain ⟨x, hxord, hxsq⟩ := hx
      exact exists_K8_normal_subgroup hn z hz hz_center x hxord hxsq
    · -- Every x of order 4 has x^2 = z
      ...
      have hk_le : k ≤ 4 := by
        ...
        exact absurd this (by decide)
      have hy_ord_2_or_4 : orderOf y = 2 ∨ orderOf y = 4 :=
        orderOf_2_or_4_of_no8 hk hk_le hy_ne_one (fun ⟨y, hy⟩ => h_order_8 ⟨y, hy⟩)
