...
/-! ### Centre of a semidirect product of abelian groups

For abelian groups `N` and `H` and a homomorphism `f : H →* MulAut N`, the
centre of the semidirect product `N ⋊_f H` is isomorphic to the product
`fixedPointsSubgroup f × f.ker`. The proof only uses that `N` and `H` are
abelian. -/
...
/-- Centre of `N ⋊_f H` for `N, H` abelian: `(n, h) ∈ Z(G)` iff
`h ∈ Ker(f)` and `Im(f)` fixes `n`. -/
...
/-- The centre of `N ⋊_f H` is isomorphic to `Fix(Im f) × Ker f`,
for any abelian groups `N` and `H`. -/
