      ...
      · -- φ nontrivial: transport to φ' : C_2 × C_2 →* Aut(C_q), show |Im(φ')| = 2
        ...
        have h_range_dvd_2 : Nat.card φ'.range ∣ 2 :=
          calc Nat.card φ'.range
            ...
            _ = Nat.gcd 4 (q - 1) := by
                rw [Nat.card_prod, card_cyclicGroup, card_mulAut_cyclicGroup_prime]
...
theorem classification_4q_q_one_mod_four
    {q : ℕ} [h_q_prime : Fact q.Prime] [Group G]
    (h_ge_3 : q > 3) (h_1_mod_4 : q ≡ 1 [MOD 4]) (h : Nat.card G = 4 * q)
    : Nonempty (G ≃* CyclicGroup (4 * q))
      ∨ Nonempty (G ≃* CyclicGroup 2 × CyclicGroup 2 × CyclicGroup q)
      ...
      have : Nonempty (G ≃* CyclicGroup (4 * q)) := by
        ...
        exact h_iso_g_p_k.symm.trans
          (h_sdp_prod.trans
            ((h_c4.some.prodCongr eK).trans (CyclicGroup.prodMulEquiv h4q)))
      exact Or.inl this
    · -- case h_c2_c2 : ↥↑P ≃* CyclicGroup 2 × CyclicGroup 2
      have h_aut_dih : Nonempty (MulAut P ≃* DihedralGroup 3) := by
        obtain ⟨e1⟩ := h_c2_c2
        obtain ⟨e2⟩ := aut_of_CpCp 2
        exact ⟨((MulAut.congr e1).trans e2).trans GL2F2_isoS3⟩
  ...
  · -- case h_nq_1 : n_q = 1
    ...
    · -- case h_K_C4 : ↥K ≃* CyclicGroup 4
      ...
      obtain ⟨r, hr_iso, _⟩ :=
        classify_sdp (p := 2) (q := q) (by omega) (by omega) 2 1
          ...
          (hK := hK_card.trans (by norm_num : (4 : ℕ) = 2 ^ 2))
          φ
      -- For q ≡ 1 (mod 4): 2 ≤ (q-1).factorization 2, so min 2 ((q-1).factorization 2) = 2
      have h_min_eq_2 : min 2 ((q - 1).factorization 2) = 2 := by
        have h_vp := two_le_factorization_two_of_prime_one_mod_four h_1_mod_4
        omega
      have h_r012 : r.val = 0 ∨ r.val = 1 ∨ r.val = 2 := by
        have h := r.isLt
        have : r.val < min 2 ((q - 1).factorization 2) + 1 := h
        omega
      have h4q : Nat.Coprime 4 q :=
        ((by norm_num : (2 : ℕ).Prime).coprime_of_ne h_q_prime.out (by omega)).pow_left 2
      ...
      · -- r = 2: G ≃* C_q ⋊_{canonicalC4OnCqAction_r2} C_4
        obtain ⟨e⟩ := hr_iso
        have : Nonempty (G ≃* SemidirectProduct (CyclicGroup q) (CyclicGroup 4)
                (canonicalC4OnCqAction_r2 h_1_mod_4)) :=
          ⟨h_iso_g_q_k.symm.trans (e.trans
    ...
    · -- case h_K_C2C2 : ↥K ≃* CyclicGroup 2 × CyclicGroup 2
      by_cases h_phi_triv : φ = 1
      · -- φ trivial: G ≃* Q × K ≃* C_q × (C_2 × C_2) ≃* (C_2 × C_2) × C_q ≃* C_2 × C_2 × C_q
        have : Nonempty (G ≃* CyclicGroup 2 × CyclicGroup 2 × CyclicGroup q) := by
          refine ⟨?_⟩
          ...
          exact h_iso_g_q_k.symm.trans
            (h_sdp_prod.trans
              ((eQ.prodCongr h_K_C2C2.some).trans
                (MulEquiv.prodComm.trans MulEquiv.prodAssoc)))
        exact Or.inr (Or.inl this)
      · -- φ nontrivial: transport to φ' : C_2 × C_2 →* Aut(C_q)
        let eK := h_K_C2C2.some
        let φ' : CyclicGroup 2 × CyclicGroup 2 →* MulAut (CyclicGroup q) :=
          ((MulAut.congr eQ).toMonoidHom).comp (φ.comp eK.symm.toMonoidHom)
        have h_sdp_congr :
        ...
        have hφ'_ne : φ' ≠ 1 := by
          ...
          rw [h2] at h1
          exact (MulEquiv.map_eq_one_iff (MulAut.congr eQ)).mp h1
        -- |Im(φ')| ∣ 2 via cyclicity + exponent 2 (since C_2 × C_2 has exponent 2)
        have h_range_dvd_2 : Nat.card φ'.range ∣ 2 := by
          -- φ'.range is a subgroup of the cyclic group MulAut (CyclicGroup q), hence cyclic
          haveI : IsCyclic (MulAut (CyclicGroup q)) := isCyclic_mulAut_cyclicGroup_prime
          haveI : IsCyclic (φ'.range : Subgroup _) := Subgroup.isCyclic _
          -- Every element of φ'.range has order dividing 2
          -- since for any x in C_2 × C_2, x^2 = 1
          have h_sq : ∀ y : φ'.range, y ^ 2 = 1 := by
            rintro ⟨y, hy⟩
            obtain ⟨x, hx⟩ := MonoidHom.mem_range.mp hy
            have hx2 : x ^ 2 = 1 := by
              obtain ⟨a, b⟩ := x
              have h_two_zmod : ∀ z : CyclicGroup 2, z ^ 2 = 1 := by
                intro z
                have h := pow_card_eq_one' (G := CyclicGroup 2) (x := z)
                rwa [card_cyclicGroup] at h
              change (a ^ 2, b ^ 2) = (1, 1)
              rw [h_two_zmod a, h_two_zmod b]
            apply Subtype.ext
            show y ^ 2 = 1
            rw [← hx, ← map_pow, hx2, map_one]
          -- φ'.range cyclic and every element squared is 1 → |φ'.range| ∣ 2
          obtain ⟨g, hg⟩ := IsCyclic.exists_generator (α := (φ'.range : Subgroup _))
          have hg_sq : g ^ 2 = 1 := h_sq g
          have h_ord_g : orderOf g ∣ 2 := orderOf_dvd_of_pow_eq_one hg_sq
          have h_card_eq : Nat.card (φ'.range : Subgroup _) = orderOf g := by
        ...
        have : Nonempty (G ≃* SemidirectProduct (CyclicGroup q) (CyclicGroup 2 × CyclicGroup 2)
                                 (canonicalC2C2OnCqAction h_ge_3)) :=
          ⟨h_iso_g_q_k.symm.trans (h_sdp_congr.trans e_canon_to_phi'.symm)⟩
        exact Or.inr (Or.inr (Or.inr (Or.inr this)))
