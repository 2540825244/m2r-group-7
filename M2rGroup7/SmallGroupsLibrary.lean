...
lemma aut_of_cyclic_p2 {p : ℕ} [h_p_prime : Fact p.Prime] :
  ...
  -- There is a theorem that says MulAut (CyclicGroup (p ^ 2)) = Cp (p - 1) (or similar)
  -- Generator of that = g
  -- There is an element of order p = g ^ (p - 1) = h
  -- Send the generator of Cp to h
